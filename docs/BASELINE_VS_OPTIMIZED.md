...
| 链路编码 | 源宿节点表示求和 + 原始容量 | 端点和、端点差、归一化容量，再投影至 24 维 | 是 |
...
源、宿节点 embedding 相加后，反向链路很难从表示中区分。优化版在 [`frameworks/hattrick_system.py`](../frameworks/hattrick_system.py) 中构造紧凑原始表示：

```text
[h_src + h_dst, h_src - h_dst, capacity / mean(abs(capacity))]
```

三层 GNN 下节点表示为 11 维，因此原始链路表示为 23 维。随后通过可学习的线性层 `Linear(23, 24)` 投影为 Set Transformer 的 24 维输入。和项与差分项可分别通过 `(sum + diff) / 2`、`(sum - diff) / 2` 恢复源、宿节点表示，因此去除了旧 34 维编码中的冗余而没有损失端点信息。

具体改进包括：

- 用和项保留两个端点的共同属性，并完整保留端点信息；
- 使用 `h_src-h_dst` 表示方向；
- 用当前图的平均绝对容量归一化链路容量，并以 `1e-6` 下界防止除零；
- 用 `23 -> 24` 可学习投影满足多头注意力的整除约束；
...
在默认自动配置下，旧 34 维方向编码只能选择 2 个注意力头，新 24 维编码会选择 6 个头。该改动减少了 Transformer 和后续 MLP 的维度，但新增了投影层；优化模型必须重新训练，不能直接复用旧方向编码模型的权重。本次实验也尚未完成“等参数量方向编码”消融。
